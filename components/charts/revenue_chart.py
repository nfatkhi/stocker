# components/charts/revenue_chart.py - Updated to match FCF layout with YoY analysis
...
class RevenueChart(FinancialBarChart):
    """Revenue chart with YoY analysis matching FCF tab layout"""
    ...
    def create_chart(self, financial_data: List[Any]) -> bool:
        """Create 3-section scrollable Revenue analysis with enhanced scrolling"""
    ...
    def _process_data(self, raw_data: List[Any]) -> List[RevenueDataPoint]:
        """Process raw data into structured format with YoY calculations"""
        data_points = []
        
        # Extract revenue data
        for financial in reversed(raw_data[:self.MAX_QUARTERS]):
            revenue = float(financial.revenue) if hasattr(financial, 'revenue') and financial.revenue > 0 else float('nan')
        ...
        return [d for d in processed if not math.isnan(d.revenue_dollars)]
    ...
    def _create_scrollable_layout(self, data: List[RevenueDataPoint]) -> bool:
        ...
        self._create_revenue_section(content_frame, data)
    ...
    def _create_summary_section(self, parent: tk.Frame, data: List[RevenueDataPoint]):
        """Create summary cards"""
        frame = tk.Frame(parent, bg=self.colors['frame'], relief='solid', bd=2, height=100)
        frame.pack(fill='x', pady=(0, 10))
        frame.pack_propagate(False)
        
        tk.Label(frame, text=f"{self.ticker} - Revenue Summary", font=(UI_CONFIG['font_family'], 12, 'bold'),
    ...
    def _create_revenue_section(self, parent: tk.Frame, data: List[RevenueDataPoint]):
        """Create main revenue chart"""
        frame = tk.Frame(parent, bg=self.colors['frame'], relief='solid', bd=2, height=400)
        frame.pack(fill='both', expand=True, pady=(0, 10))
        frame.pack_propagate(False)
        
        tk.Label(frame, text="Quarterly Revenue Trend", font=(UI_CONFIG['font_family'], 14, 'bold'),
                bg=self.colors['frame'], fg=self.colors['header']).pack(pady=(10, 5))
        
        content = tk.Frame(frame, bg=self.colors['bg'])
        content.pack(fill='both', expand=True, padx=10, pady=(0, 10))
        
        values = [d.revenue_dollars for d in data]
        quarters = [d.quarter_label for d in data]
        
        self.create_interactive_chart(content, values, quarters, f"{self.ticker} Quarterly Revenue", 
                                     "Quarterly Revenue", self.COLORS['positive'], "Revenue")
    ...
    def _create_yoy_chart(self, parent: tk.Frame, yoy_data: List[RevenueDataPoint]):
        ...
        try:
            ...
            # Get sizing and fonts to match revenue chart
            ...
            # Style chart to match revenue chart formatting
            ...
            # Reference lines with consistent styling
    ...
    def _show_yoy_fallback(self, parent: tk.Frame, yoy_data: List[RevenueDataPoint], error: str):
...
class RevenueTab:
    """Revenue tab container with enhanced scrolling support"""
    ...
    def update_data(self, financial_data: List[Any], ticker: str = None, data_source_info: Dict = None):
        ...
        try:
            ...
            tk.Label(self.parent_frame, text=f"❌ Revenue Error\n\n{e}",
...
print("📊 Enhanced Revenue Chart loaded with YoY analysis and optimized scrolling")